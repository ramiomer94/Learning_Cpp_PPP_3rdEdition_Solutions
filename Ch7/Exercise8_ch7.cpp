...
/*
 * Purpose:
 *  - Prompts the user to enter names (alphabetic only) until the user enters the keyword "done".
 ...
 * 
 * Returns:
 *  - vector<string> containing arbitrary number of names.
 */
vector<string> collect_names (){
    ...
    while(true){
        ...
        // If the character after the name isn't whitespace, input is invalid
        if(!isspace(c)) error("expected a string of alphabetic characters");
         // If user types "done", stop reading names
        if(s == "done") break;
