...
double factorial() {
    ...
    if (t.kind == '!') {
        // Only allow ONE '!' — peek ahead
        Token next = ts.get();
        if (next.kind == '!') {
            error("Double factorial (!!) not supported.");
        } else {
            ts.putback(next);  // if not another '!', put it back
        }

        int n = static_cast<int>(left);
        if (n == 0) {
            return 1;
        }
    ...
    // No factorial, just return the original value
    ts.putback(t);
    return left;
}
