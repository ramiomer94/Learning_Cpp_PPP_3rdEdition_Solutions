/* Suggested Improvements:
    ...
    1- History feature (last result recall with ‘ans’):
         Allow users to refer to the result of the previous calculation using a keyword like 'ans'.
    
    2- Variable deletion: Allow users to delete variables they’ve declared.
        How: Add a new command like del x; that removes variable x from the symbol table.

    3- Add a clear command to reset variables: Allow users to reset the calculator by clearing all 
        user-defined variables but keeping constants.
    
    The first improvement (history feature) has been implemented below
*/
